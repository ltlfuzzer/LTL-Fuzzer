...
u8 common_subject = 0;
u8 protocol_subject = 0;

PREFIX_SMEM* prefix_shm;
...
/* protocol-specific variables & functions */
...
klist_t(lms) *add_prefix_protocol(){
  klist_t(lms) *kl_messages_add = kl_init(lms);
  uint32_t prefix_size = prefix_shm->arr_size;

  for(uint32_t i = 0; i < prefix_size; i++){
    message_t *m = (message_t *) ck_alloc(sizeof(message_t));
    m->msize = strlen((prefix_shm->array)[i]);
    if(m->msize == 0) break;
    m->mdata = (char *) ck_alloc(m->msize);
    if(m->mdata == NULL) {
        break;
    }
    memcpy((char*)(m->mdata), (prefix_shm->array)[i], (m->msize)*sizeof(char));
    *kl_pushp(lms, kl_messages_add) = m;      
  }
  
  return kl_messages_add;
}
...
int send_over_network()
{
  ...
  if(prefix_shm != (PREFIX_SMEM*)-1){
    klist_t(lms) *kl_messages_add = add_prefix_protocol();
	  kliter_t(lms) *it_add;
	  for (it_add = kl_begin(kl_messages_add); it_add != kl_end(kl_messages_add); it_add = kl_next(it_add)) {
      net_send(sockfd, timeout, kl_val(it_add)->mdata, kl_val(it_add)->msize);
      messages_sent++;
    }
  }
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    messages_sent++;
    strcat(input_buffer, kl_val(it)->mdata);
    index[counter++] = kl_val(it)->msize;
    ...
    //set likely_buggy flag if protocol does not receive any feedback from the server
...
/* End of protocol-specific variables & functions */
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  if(protocol_subject){
    /* protocol: extract regions keeping client requests if needed */
    if (corpus_read_or_sync) {
      ...
      if (byte_count != len) PFATAL("protocol - Inconsistent file length '%s'", fname);
...
EXP_ST void destroy_queue(void) {
  ...
  while (q) {
    ...
    if(protocol_subject){
      u32 i;
      //Free protocol-specific data structure
...
static void read_testcases(void) {
  ...
  if(protocol_subject){
    /* protocol: set this flag to enable request extractions while adding new seed to the queue */
  ...
  if(protocol_subject){
    /* protocol: unset this flag to disable request extractions while adding new seed to the queue */
...
/* Write modified data to file for testing. If out_file is set, the old file
   ...
   truncated. */
//REPLACE_PREFIX
//void replace_with_prefix(void** mem, uint32_t *len);

static void write_to_testcase(void* mem, u32 len) {
  //if(getenv("LTL") != NULL){
       // printf("---replace input prefix---");
        //replace_with_prefix(&mem, &len);
  //} 
  if(common_subject){
    if(prefix_shm != (PREFIX_SMEM*)-1){
      add_prefix_common(&mem, &len, prefix_shm);
    }
  }
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    /* protocol construct the kl_messages linked list for this queue entry*/
    if(protocol_subject){
    ...
    /* protocol delete the kl_messages */
    if(protocol_subject){
...
keep_as_crash:
  ...
  ck_free(fn);
  if(unique_crashes > 0){
      system("pkill ltl-fuzz && pkill afl-fuzz");
  }
  return keeping;
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if(protocol_subject){
    /* protocol update kl_messages linked list */
    // parse the out_buf into messages
    u32 region_count;
    region_t *regions = (*extract_requests)(out_buf, len, &region_count);
    if (!region_count) PFATAL("protocol Region count cannot be Zero");
...
static u8 fuzz_one(char** argv) {
  ...
  s32 len = 0, fd, temp_len, i, j;
  u8  *in_buf = NULL, *out_buf = NULL, *orig_in, *ex_tmp, *eff_map = 0;
  u64 havoc_queued,  orig_hit_cnt, new_hit_cnt;
  u32 splice_cycle = 0, perf_score = 100, orig_perf, prev_cksum, eff_cnt = 1, M2_len = 0;
  ...
  if(protocol_subject){
    ...
    /* Identify the prefix M1, the candidate subsequence M2, and the suffix M3. See protocol paper */
    ...
    while (it != M2_next) {
      ...
      if (!in_buf) PFATAL("protocol cannot allocate memory for in_buf");
...
retry_splicing:
  ...
  if (use_splicing && splice_cycle++ < SPLICE_CYCLES && queued_paths > 1 && ((common_subject && queue_cur->len >1) || (protocol_subject && M2_len >1))) {
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "Settings for network protocol fuzzing (protocol):\n\n"
...
void init_shm(){
  int prefix_shm_id = get_prefix_smem();
  if(prefix_shm_id == -1){
    prefix_shm = (PREFIX_SMEM*)-1;
    printf("Failed to get the prefix shm... \n");
  }
  else{
    prefix_shm = bind_prefix_smem(prefix_shm_id);
  }
}
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:P:FKz:c:A")) > 0)
    ...
    switch (opt) {
      ...
      case 'P': /* protocol to be tested */
        ...
        if (!strcmp(optarg, "RTSP")) {
          ...
          extract_response_codes = &extract_response_codes_tls;
		    ...
		    else if (!strcmp(optarg, "TELNET")) {
          extract_requests = &extract_requests_telnet;
          extract_response_codes = &extract_response_codes_telnet;
        } else {
      ...
      case 'A': /* crash mode */
        ...
        if (common_subject) FATAL("Multiple -C options not supported");
        common_subject = 1;
        break;
  ...
  //protocol - Check for required arguments
  if(protocol_subject){
